...
class GeorefPageContractTests(unittest.TestCase):
    ...
    def test_atlas_manual_export_is_unambiguously_unqualified(self):
        ...
        self.assertIn("const pointRenderer = L.canvas({ pane: 'sitePointPane', padding: 0.5, tolerance: 12 })", javascript)
        self.assertIn("const localityRenderer = pointRenderer", javascript)
        self.assertIn("const siteRenderer = pointRenderer", javascript)
        self.assertIn("map.getPane('sourcePhotoPane').style.zIndex = '510'", javascript)
        ...
        self.assertIn(".source-photo-anchor-target", stylesheet)
        ...
        self.assertIn("iconSize: [hitboxWidth + 16, 48]", javascript)
        self.assertIn('class="source-photo-anchor-target"', javascript)
        self.assertIn("marker.bindPopup(sourcePhotoChoicePopup(records)", javascript)
